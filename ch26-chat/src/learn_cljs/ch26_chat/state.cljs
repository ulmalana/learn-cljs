...
(defn is-current-view-conversations? [state]
...
(defn room-list [state]
  (let [current-room (when (is-current-view-room? state)
                       (get-in state [:current-view :id]))]
    (map (fn [room]
           (assoc room :active? (= current-room (:id room))))
         (:rooms state))))

(defn people-list [app]
  (let [current-username (when (is-current-view-conversations? app)
                           (get-in app [:current-view :username]))]
    (map (fn [person]
           (assoc person :active? (= current-username (:username person))))
         (:people app))))
...
(defn auth-modal-toggled [state]
  (update state :auth-modal {:sign-up :sign-in
                             :sign-in :sign-up}))
